...
- In the `icon` slot, use `eds-icon` with the Material icon set for the application glyph.
- In the `breadcrumb` slot, use `eds-breadcrumb` with a separator (`<span slot="separator">/</span>`) and `eds-breadcrumb-item` entries.
...
- In `eds-page-info`, place optional state badges in the `badge` slot using `eds-badge`.
- In the `center` slot, use `eds-tab-group` for application-level tabs. Keep tab labels concise and noun-based.
- In the `right` slot, use concise utility actions such as `eds-button` and `eds-icon-button`.
...
- When using `eds-application-layout` with a left side panel (`slot='left-panel'`), include a view-sidebar icon button in the page header: `<eds-icon-button slot="icon" library="material" name="view_sidebar" label="View sidebar"></eds-icon-button>`.
...
- Use approved card components (`eds-card`, `eds-selectable-card`, `eds-catalog-card`) based on behavior.
...
- Keep typical `eds-input`, `eds-select`, and other form controls at `max-width: 30rem`; do not use unconstrained `width: 100%` on the controls unless the design explicitly requires full-width fields.
...
- The right `actions` area includes a search icon button, one content-width `eds-button` with `variant="primary"`, and an overflow menu icon button, in that order. Inputs and buttons use their default medium size.
- Use `eds-icon-button` for familiar icon actions and provide an accessible `label` for every icon button.
...
- When the search icon button is selected, replace it with a standard-size `eds-input` using the placeholder `Search...` and keep focus in the input.
...
- Always set the `label` attribute on `sl-radio-group`, except for the unlabeled `sl-radio-button` tabs alternative described below; use an accessible name such as `aria-label` there.
- Set a `value` attribute on each `sl-radio` to identify the selected option.
- Use `sl-radio-button` as a tabs alternative when switching related content, rather than presenting a completely new UI. In this pattern, omit the visible group label and use an accessible name such as `aria-label`.

## Buttons

- Use `eds-button` for button actions unless an explicit requirement states otherwise.
...
- Do not compose a selectable tile manually from `eds-card` plus a separate checkbox or switch unless a documented component limitation requires it.
- Use generic `eds-card` only for non-selectable, presentational containers.
- Use approved card components (`eds-card`, `eds-selectable-card`, `eds-catalog-card`) based on behavior.
- Use `var(--sl-spacing-large)` for both horizontal and vertical spacing between cards (for example, `column-gap` and `row-gap` in card grids).
...
- Use `eds-appbar` for app-level chrome (breadcrumb, page info, app actions); use `eds-page-header` for page-level headings inside content regions.
- Use `eds-panel-layout` for panel content that must stay contained inside layout slots.
...
- Open dialogs with `show()` only on explicit user actions.
...
- Always include a footer with two buttons in this order:
  1. `<eds-button>` with variant `default` for "Cancel"
  2. `<eds-button>` with variant `primary` for the confirmation action (for example, "Add", "Save", "Delete")
- Use a `<div slot="footer">` wrapper for footer actions with flex layout for right alignment.

## Tabs

- Use `eds-tab-group` with `eds-tab` only.
- Never use `eds-tab-panel` components.
...
- To add a new sidebar section, place `<eds-divider></eds-divider>` after the previous group, then add `<eds-sidenav-item type="heading" label="Section name"></eds-sidenav-item>` before that section's items.
- Keep a single source of truth for nav items and synchronize active state with the current route.

## Tree

- Use `eds-tree` with `eds-tree-item` for hierarchical tree navigation. Do not build custom tree controls from buttons or divs.
- Set `selection="single"` on `eds-tree` for single-item selection; use "multiple" or "leaf" only when the interaction explicitly requires it.
- Add a `value` attribute to each `eds-tree-item` to identify the node on selection and query events.
- Nest `eds-tree-item` elements directly inside a parent `eds-tree-item` to create child levels; `eds-tree` manages expand/collapse controls automatically.
- Do not set `expanded` or `selected` as declarative React props; set initial values imperatively after mount when needed.
- Listen to `eds-selection-change` on the `eds-tree` ref to update React state.
...
- Use the AspenTech assistance package components and register each component before rendering it: `ava-layout`, `ava-input`, `ava-welcome-message`, `ava-user-message`, and `ava-response-message`.
- Launch AVA from an app-bar action using `<ava-logo variant="isotype"></ava-logo>` inside an accessible button.
- Render AVA in a right-side panel opened only by that explicit app-bar action; keep the panel closed by default and provide a close action.
- Use `ava-layout` as the primary container for AVA chat-style experiences.
- Use `header-actions` slot for utility actions such as New chat (`eds-icon-button`, optionally wrapped with `eds-tooltip`).
...
### In-UI Alerts
- Use `eds-alert` for alert messages.
...
- Keep toasts non-blocking and use accessible, concise content.
- Imperative usage is required: create the element on trigger with `document.createElement('eds-toast')` and append it; never render toast as static markup.
