...
- Use `eds-shell-template` without a fixed sidenav.
- Put `eds-appbar` in the `appbar` slot with `show-menu-button` and `is-application`.
- Add the application glyph as a Material `sl-icon` in the `icon` slot.
- Add `sl-breadcrumb` in the `breadcrumb` slot with `<span slot="separator">/</span>` and `sl-breadcrumb-item` entries.
- Add `eds-page-info` in the `page-info` slot. Put optional menu items in `heading-menu-items` and optional badges in `badge`.
- Put application-level navigation in the `center` slot as `sl-tab-group` with `sl-tab` children only. Use short, descriptive noun labels such as `Overview`, `Activity`, and `Settings`; keep the active tab synchronized with the main content.
- Put concise utility actions, such as `sl-button` and `sl-icon-button`, in the `right` slot.
- Add a hidden-by-default modal `eds-sidenav` with `mode="modal"`. Open it only from the built-in appbar menu button with `sidenav.show()`; do not add a competing menu button.
- Mirror the primary navigation in the modal sidenav. On item selection, navigate and call `sidenav.hide()`. Preserve the default Esc, outside-click, and focus behavior.
...
- When using `eds-application-layout` with a left side panel (`slot='left-panel'`), include an accessible button in the page header `icon` slot containing `<sl-icon library="material" name="view_sidebar"></sl-icon>`.
...
- When a right panel is used for an action, wrap it in `eds-panel-layout` with a meaningful heading and `closable`; handle its `eds-close` event by removing `open-right` from the parent `eds-application-layout`.
...
- Place content inside the `page-content` container.
...
- Do not apply the EDS-only `--flex-direction` and `--nav-order` overrides to `sl-tab-group`.
...
- Listen to `eds-selection-change` on the `sl-tree` ref to update React state.
...
### In-UI Alerts
- Use `sl-alert` for alert messages.
- Set alerts to stretch to 100% of their containing section.
- Place `sl-alert` above the section it relates to, typically at the top of the page below the page header or at the top of a dialog body.
- Do not use the `heading` property in `sl-alert`.
- Always set `closable` on `sl-alert` elements with the `primary`, `neutral`, or `warning` variant.
- Never set `closable` on an `sl-alert` with the `danger` variant.
